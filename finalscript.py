...
# Load data
responses_df = pd.read_csv('csv_files/responses.csv')
lexicon_df = pd.read_csv('csv_files/lexdict.csv')
model_df = pd.read_csv('csv_files/model_parameters.csv')
...
def generate_wordcloud_positive():
    try:
        fig, ax = plt.subplots(figsize=(7, 6))
        plt.subplots_adjust(left=-0.1, right=0.9, top=0.9, bottom=0, hspace=0.4)
        # plt.imshow(WordCloud(width=400, height=400, background_color='white', 
        #                       prefer_horizontal=1.0).generate(positive_words if positive_words else "No Positive Words"))
        wordcloud = WordCloud(width=450, height=340, background_color='white', 
                              prefer_horizontal=1.0).generate(positive_words if positive_words else "No Positive Words")

        ax.imshow(wordcloud, interpolation='bilinear')
        plt.title('Positive Words', loc="left", fontsize=16, fontweight="bold")
        plt.axis('off')
        plt.tight_layout()
        ...
        return {"column": "Q9", "image": img_base64}
    except Exception as e:
        logging.error(f"Error generating pie charts: {e}")
        return []

def generate_wordcloud_negative():
    try:
        fig, ax = plt.subplots(figsize=(7, 6))

        wordcloud = WordCloud(width=450, height=340, background_color='white', 
                              prefer_horizontal=1.0).generate(negative_words if negative_words else "No Negative Words")

        ax.imshow(wordcloud, interpolation='bilinear')
        ax.set_title('Negative Words', loc="left", fontsize=16, fontweight="bold")
        ax.axis('off')
        plt.tight_layout()
        ...
        return {"column": "Q12", "image": img_base64}
    except Exception as e:
